# Custom servie to start application automatically for webrtc

This custom forked from `firebase_messaging: 7.0.3`

### From plugin android native

`android/src/main/java/io/flutter/plugins/firebasemessaging/CustomFlutterFirebaseMessagingService.java`
```
...
<manifest xmlns:android="http://schemas.android.com/apk/res/android"
  ...
  <application>
    <service android:name=".CustomFlutterFirebaseMessagingService">
      <intent-filter>
...
Config fcm `cfg_fcm.dart`

```
import 'dart:io';

import 'package:firebase_messaging/firebase_messaging.dart';
import 'package:goferphone/services/local/storage.dart';
import 'package:goferphone/services/local/storage_preferences.dart';
import 'package:goferphone/utils/app_helper.dart';
import 'package:goferphone/utils/app_log.dart';

class CfgFcm {
  CfgFcm._private();

  // ignore: sort_unnamed_constructors_first
  factory CfgFcm() => instance;
  static final CfgFcm instance = CfgFcm._private();
  ...
  void configFCM() {
    _firebaseMessaging.configure(
      // On message will on in foreground mode and in recent list
      // In-case terminated use loadRemoteMessage() instead
  ...
  // Load remote message
  // Call this message below configFCM() places at MyAppState initState()
  // When app in background:
  // - Flutter service will store remote data message to preferences
  // - Flutter service will start main activity
  // - The app will read remote data message from preferences
  Future<void> loadRemoteMessage() async {
    if (Platform.isAndroid) {
      final Storage storage = StoragePreferences();
      const String key = 'pref_remote_message_data_key';
      await storage.reload();
      final String value = await storage.getData<String>(key);
      if (value != null) {
        await storage.saveData<String>(key, null);
        logger.d('remote message: $value');
        AppHelper.showToast('remote message: $value');
      }
    }
  }
}
...
class _MyAppState extends State<MyApp> {
  ...
  void initState() {
    ...
    // Init fcm
    widget.fcm.configFCM();

    // Load remote message
    widget.fcm.loadRemoteMessage();
  }
